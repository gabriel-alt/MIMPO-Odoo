...
QUnit.module("spreadsheet > pivot_autofill", {}, () => {
    ...
    QUnit.test("Autofill pivot values with date in cols", async function (assert) {
        ...
        setCellContent(model, "C2", '=ODOO.PIVOT.HEADER(1,"date:day","04/15/2016")');
        assert.strictEqual(
            getPivotAutofillValue(model, "C2", { direction: "bottom", steps: 1 }),
            '=ODOO.PIVOT.HEADER(1,"date:day","04/15/2016","measure","probability")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "C2", { direction: "bottom", steps: 2 }),
            '=ODOO.PIVOT(1,"probability","foo",1,"date:day","04/15/2016")'
        );
    });

    QUnit.test("Autofill pivot values with date in cols and multiple cols", async (assert) => {
        const { model } = await createSpreadsheetWithPivot({
            arch: /*xml*/ `
                <pivot>
        ...
        setCellContent(model, "H2", '=ODOO.PIVOT.HEADER(1,"date:month","05/2016")');
        assert.strictEqual(
            getPivotAutofillValue(model, "H2", { direction: "bottom", steps: 1 }),
            ""
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "H2", { direction: "bottom", steps: 2 }),
            ""
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "H2", { direction: "bottom", steps: 3 }),
            ""
        );
    });
