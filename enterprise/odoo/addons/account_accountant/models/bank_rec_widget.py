...
class BankRecWidget(models.Model):
    ...
    journal_default_account_id = fields.Many2one(
        related='st_line_id.journal_id.default_account_id',
        depends=['st_line_id'],
    )
    ...
    form_account_id = fields.Many2one(
        ...
        domain="[('id', '!=', journal_default_account_id), ('account_type', 'not in', ('asset_cash', 'off_balance')), ('company_id', '=', company_id), ('deprecated', '=', False)]",
    )
    ...
    def _process_todo_command(self, command_name, command_args):
        ...
        if command_name == 'trigger_matching_rules':
            ...
            get_public_method(self, method_name)(self, *command_args[1:])
    ...
    def _do_amounts_apply_for_early_payment(self, open_amount_currency, total_early_payment_discount, transaction_currency):
        return transaction_currency.compare_amounts(open_amount_currency, total_early_payment_discount) == 0
    ...
    def _lines_widget_check_apply_early_payment_discount(self):
        ...
        if is_same_currency \
            ...
            and self._do_amounts_apply_for_early_payment(open_amount_currency, total_early_payment_discount, self.transaction_currency_id):
    ...
    def _get_line_create_command_dict(
        ...
        if line.flag == 'early_payment':  # Used to compute tax_tag_invert for early payment discount lines
    ...
    def js_action_reconcile_st_line(self, st_line_id, params):
        ...
        for index, counterpart_aml_id in params['to_reconcile']:
            ...
            rec_res = (line_aml + counterpart_aml).with_context(no_exchange_difference_no_recursive=True).reconcile()
    ...
    def collect_global_info_data(self, journal_id):
        ...
        return {
            'balance_amount': currency.format(journal.current_statement_balance),
        }
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        # EXTENDS base
        # include field 'journal_default_account_id' to avoid a module update (required to compute account domain)
        # TO BE REMOVED WHEN BANK RECO CONVERTED TO OWL
        res = super().get_view(view_id=view_id, view_type=view_type, options=options)
        if view_type == 'form':
            form_view = self.env.ref('account_accountant.view_bank_rec_widget_form')
            tree = etree.fromstring(res['arch'])
            if res.get('id') == form_view.id and len(tree.xpath("//field[@name='journal_default_account_id']")) == 0:
                arch_tree = etree.fromstring(form_view.arch)
                journal_currency_field = arch_tree.find("field[@name='journal_currency_id']")
                journal_currency_field.addnext(
                    etree.Element(
                        'field',
                        attrib={
                            'name': 'journal_default_account_id',
                            'invisible': '1',
                        },
                    )
                )
