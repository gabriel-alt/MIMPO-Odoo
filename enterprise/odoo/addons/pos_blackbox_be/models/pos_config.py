...
class PosConfig(models.Model):
    ...
    certified_blackbox_identifier = fields.Char('Blackbox Identifier', store=True, compute="_compute_certified_pos", readonly=True)

    @api.depends('iface_fiscal_data_module')
    def _compute_certified_pos(self):
        if self.iface_fiscal_data_module:
            self.certified_blackbox_identifier = self.iface_fiscal_data_module.name[-14:]
            self.env["pos_blackbox_be.log"].sudo().create("Session started with: %s" % self.certified_blackbox_identifier, "create", self._name, self.name)

    def _check_is_certified_pos(self):
        if self.certified_blackbox_identifier and not self.iface_fiscal_data_module:
            raise UserError(_("Forbidden to start a certified Point of sale without blackbox"))
    ...
    def _check_blackbox_config(self):
        loyalty = self.env['ir.module.module'].search([('name', '=', 'pos_loyalty'), ('state', '=', 'installed')])
        for config in self:
            if config.iface_fiscal_data_module and (config.module_pos_discount or (loyalty and config._get_program_ids())):
                raise UserError(_("Loyalty programs, gift card, reprint and global discounts cannot be used on a PoS associated with a blackbox."))
    ...
    def _check_employee_insz_or_bis_number(self):
        for config in self:
            if config.module_pos_hr:
                emp_names = []
                for emp in config.employee_ids if config.employee_ids else self.env['hr.employee'].search([]):
                    if not emp.insz_or_bis_number:
                        emp_names.append(emp.name)
                if not self.env.user.employee_id.insz_or_bis_number:
                    emp_names.append(self.env.user.name)

                if len(emp_names) > 0:
                    raise ValidationError(_("%s must have an INSZ or BIS number.", ", ".join(emp_names)))

    def _check_cash_rounding(self):
        if not self.cash_rounding:
            raise ValidationError(_("Cash rounding must be enabled"))
        if self.rounding_method.rounding != 0.05 or self.rounding_method.rounding_method != "HALF-UP":
            raise ValidationError(_("The rounding method must be set to 0.5 and HALF-UP"))
