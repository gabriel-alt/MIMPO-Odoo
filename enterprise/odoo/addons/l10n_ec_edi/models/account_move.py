...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ec_withhold_inv_fields(self):
        ...
        grouped_lines = self.env['account.move.line'].read_group(
            domain=[('l10n_ec_withhold_invoice_id', 'in', invoices_ec.ids)],
            fields=['move_id:array_agg'],
            groupby=['l10n_ec_withhold_invoice_id'],
        )
        withhold_invoice_line_data = {
            group['l10n_ec_withhold_invoice_id'][0]: list(set(group['move_id'])) for group in grouped_lines
        }
        for invoice in invoices_ec:
            withhold_ids = False
            withhold_count = False
            if invoice.is_invoice():
                withhold_ids = withhold_invoice_line_data.get(invoice.id) or []
    ...
    def _l10n_ec_get_payment_data(self):
        ...
        for line in pay_term_line_ids:
            ...
            if self.invoice_payment_term_id and line.date_maturity:
                payment_vals.update({
                    'payment_term': max(((line.date_maturity - self.invoice_date).days), 0) if self.invoice_date else 0,
                    'time_unit': "dias",
                })
