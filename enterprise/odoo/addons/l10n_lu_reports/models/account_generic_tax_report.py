...
class LuxembourgishTaxReportCustomHandler(models.AbstractModel):
    ...
    def get_tax_electronic_report_values(self, options):
        ...
        ioss = hasattr(report.env.company, "ioss") and report.env.company.ioss
        values['491'] = {'value': oss and '1' or '0', 'field_type': 'boolean'}
        values['492'] = {'value': ioss and '1' or '0', 'field_type': 'boolean'}

        # 481 and 482 are the SME scheme fields
        sme = any([float(values[index]['value'].replace(',', '.')) for index in ['481', '482']])
        values['493'] = {'value': sme and '1' or '0', 'field_type': 'boolean'}
