...
class AccountPayment(models.Model):
    ...
    def _l10n_it_riba_check(self):
        errors = []
        partner2bank_accounts = self._l10n_it_riba_partner_to_bank_accounts()
        for payment in self:
            partner_bank_account = partner2bank_accounts.get(payment.partner_id.id, self.env['res.partner.bank'])[:1]
            ...
            if payment.is_internal_transfer:
                errors.append(_("One of the payments is an internal transfer."))
    ...
    def _l10n_it_riba_partner_to_bank_accounts(self):
        partner2bank_accounts = defaultdict(lambda: self.env['res.partner.bank'])
        for rpb in self.env['res.partner.bank'].search([('partner_id', 'in', self.partner_id.ids)]):
            partner2bank_accounts[rpb.partner_id.id] |= rpb
        return partner2bank_accounts
    ...
    def _l10n_it_riba_get_values(self, creditor, creditor_bank, creditor_bank_account):
        ...
        for section_number, payment in enumerate(self, 1):
            ...
            debitor_bank_account = partner2bank_accounts.get(payment.partner_id.id, self.env['res.partner.bank'])[:1]
            ...
            records += [
                {
                    ...
                    'segment_1': spaced_join(
                        ...
                        payment.ref or spaced_join(_("Invoice"), *reconciled_invoices.mapped("name")),
                        _("Amount"),
