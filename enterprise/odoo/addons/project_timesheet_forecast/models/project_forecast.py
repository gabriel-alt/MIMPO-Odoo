...
class Forecast(models.Model):
    ...
    def _get_planning_cutoff_date(self):
    ...
    def _action_generate_timesheet(self):
        ...
        cutoff_date = self._get_planning_cutoff_date()
        filter_domain = [
            ('project_id', '!=', False),
        ...
        interval_per_employee = defaultdict(lambda: (cutoff_date, datetime(1970, 1, 1)))
        ...
        for slot in slots:
            if slot.resource_id and slot.resource_id.flexible_hours:
                ...
                if end_datetime < slot.end_datetime:
                    end_datetime = min(slot.end_datetime, cutoff_date)
                interval_per_employee[slot.employee_id] = (start_datetime, end_datetime)

        min_date, max_date = pytz.utc.localize(cutoff_date), None
        ...
        for res in timesheet_read_group:
            timesheet_date = fields.Date.to_date(res['__range']['date:day']['from'])
