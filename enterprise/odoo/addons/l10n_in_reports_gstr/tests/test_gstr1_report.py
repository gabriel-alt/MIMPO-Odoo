...
from .gstr_test_json import gstr1_test_json, gstr1_test_2_json, new_hsn_schema_gstr1_test_json
import logging
...
class TestReports(TestAccountReportsCommon):
    ...
    def test_hsn_schema_change_gstr1_json(self):
        b2b_invoice = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, self.igst_18, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2b_invoice, invoice_line_vals={'quantity': 1})  # Creates and posts credit note for the above invoice

        b2b_intrastate_invoice = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_2, self.sgst_18, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2b_intrastate_invoice, invoice_line_vals={'quantity': 1})

        b2c_intrastate_invoice = self.l10n_in_reports_gstr1_inv_init(self.consumer_partner, self.sgst_18, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2c_intrastate_invoice, invoice_line_vals={'quantity': 1})

        b2cl_invoice = self.l10n_in_reports_gstr1_inv_init(self.large_unregistered_partner, self.igst_18, invoice_line_vals={'price_unit': 250000, 'quantity': 1}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2cl_invoice, invoice_line_vals={'quantity': 0.5})

        export_invoice = self.l10n_in_reports_gstr1_inv_init(self.oversea_partner, self.igst_18, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=export_invoice, invoice_line_vals={'quantity': 1})

        b2b_invoice_nilratedtax = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, self.nil_rated_tax, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2b_invoice_nilratedtax, invoice_line_vals={'quantity': 1})

        b2b_invoice_exemptedtax = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, self.exempt_tax, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2b_invoice_exemptedtax, invoice_line_vals={'quantity': 1})

        b2b_invoice_nongsttax = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, self.non_gst_supplies, invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)
        self.l10n_in_reports_gstr1_inv_init(inv=b2b_invoice_nongsttax, invoice_line_vals={'quantity': 1})

        # if no tax is applied then it will be out of scope and not considered in GSTR1
        self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, [], invoice_line_vals={'price_unit': 500, 'quantity': 2}, invoice_date=HSN_CHANGE_TEST_DATE)

        # for b2b invoice with 2 invoice_line_ids having different taxes
        b2b_invoice_gst_and_nil_rated_tax = self.l10n_in_reports_gstr1_inv_init(self.registered_partner_1, self.nil_rated_tax, invoice_line_vals={'price_unit': 700, 'quantity': 2}, post=False, invoice_date=HSN_CHANGE_TEST_DATE)
        existing_line_vals = b2b_invoice.invoice_line_ids[0].read(['product_id', 'account_id', 'price_unit', 'quantity', 'tax_ids'])[0]
        b2b_invoice_gst_and_nil_rated_tax.write({
            'invoice_line_ids': [
                Command.create({
        ...
        # b2b invoice with special economic zone
        b2b_sez_invoice_gst_and_nil_rated_tax = b2b_invoice_gst_and_nil_rated_tax.copy(default={'l10n_in_gst_treatment': 'special_economic_zone', 'invoice_date': HSN_CHANGE_TEST_DATE})
        b2b_sez_invoice_gst_and_nil_rated_tax.action_post()
        ...
        gstr1_json = gstr1_report._get_gstr1_json()
        self.assertDictEqual(gstr1_json, new_hsn_schema_gstr1_test_json)
