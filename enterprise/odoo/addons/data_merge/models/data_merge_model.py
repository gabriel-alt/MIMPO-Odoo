            ...
            for rule in dm_model.rule_ids:
                ...
                if company_field and not dm_model.mix_by_company:
                    if company_field.store or company_field.inherited:
                        ...
                        related_model = self.env[company_field.related_field.model_name]
                        related_company_field = res_model._inherits_join_calc(table, company_field.related.split(".")[0], query)
                        lhs_alias, lhs_column = re.findall(r'"([^"]+)"', related_company_field)
                        rhs_alias = query.left_join(lhs_alias, lhs_column, related_model._table, 'id', lhs_column)
                        group_by = ', %s' % res_model._inherits_join_calc(rhs_alias, 'company_id', query)
