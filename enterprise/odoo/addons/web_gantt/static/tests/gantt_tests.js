...
document.createElement("a").classList.contains
    ...
    QUnit.test("The date and task should appear even if the pill is planned on 2 days but displayed in one day by the gantt view", async function (assert) {
        ...
        initialDate = new Date(initialDate.getTime() - initialDate.getTimezoneOffset() * 60 * 1000);
        const [task9_planned_dates, task10_planned_dates, task11_planned_dates] = [
            {
        ...
        this.data.tasks.records.push({
            ...
            name: "Task 10",
            allocated_hours: 4,
            ...task10_planned_dates,
        }, {
            // will be displayed in 2 days
            id: 11,
            name: "Task 11",
            allocated_hours: 4,
            ...task11_planned_dates,
        });
        const gantt = await createView({
            View: GanttView,
            model: "tasks",
        ...
        const pills = gantt.el.querySelectorAll(".o_gantt_pill_title");
        assert.strictEqual(pills[0].innerText, "4:00 PM - 1:00 AM (4h) - Task 9", "The date should be displayed since the pill is displayed in one day.");
        assert.strictEqual(pills[1].innerText, "4:00 PM - 2:00 AM (4h) - Task 10", "The date should be displayed since the pill is displayed in one day.");
        assert.strictEqual(pills[2].innerText, "Task 11", "The date should not be displayed since the pill is displayed in 2 days.");
        gantt.destroy();
    });

    QUnit.test('date fields: domain', async function (assert) {
        assert.expect(4);
        this.data.tasks.fields.start = { string: 'Start', type: 'date' };
        this.data.tasks.fields.stop = { string: 'Stop', type: 'date' };
        ...
        const gantt = await createView({
            ...
            model: 'tasks',
            data: this.data,
            arch: `<gantt date_start="start" date_stop="stop" />`, // what about dynamic_range="1" and scale="day"?
            viewOptions: { initialDate },
            mockRPC: function (route, args) {
                if (route === '/web/dataset/search_read') {
                    assert.deepEqual(args.domain, domains.pop());
                }
                return this._super.apply(this, arguments);
            },
        });
    ...
    QUnit.test('date fields: pill columns', async function (assert) {
        ...
        this.data.tasks.fields.start = { string: 'Start', type: 'date' };
        this.data.tasks.fields.stop = { string: 'Stop', type: 'date' };
        this.data.tasks.records = this.data.tasks.records.slice(0, 1);
        this.data.tasks.records[0].start = "2018-12-20";
        this.data.tasks.records[0].stop = "2018-12-22";
        ...
        assert.containsOnce(gantt, ".o_gantt_pill");
        assert.strictEqual(gantt.el.querySelector(".o_gantt_cell:has(.o_gantt_pill)").dataset.date, "2018-12-20 00:00:00");
        assert.strictEqual(getPillItemWidth(gantt.$('.o_gantt_pill_wrapper:eq(0)')), "calc(300% + 2px - 4px)");
    ...
    QUnit.test('date fields: resize a pill', async function (assert) {
        ...
        const gantt = await createView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'write') {
                    ...
                    assert.deepEqual(args.args[1], { stop: "2018-12-21" });
                }
                return this._super.apply(this, arguments);
            },
        });
        ...
        // resize to one cell smaller (-1 day)
        let cellWidth = gantt.$('.o_gantt_cell:first').width() + 4;
        await testUtils.dom.dragAndDrop(
            gantt.$('.ui-resizable-e'),
            gantt.$('.ui-resizable-e'),
            { position: { left: -cellWidth, top: 0 } }
        ...
        assert.containsOnce(gantt, ".o_gantt_pill");
        assert.strictEqual(gantt.el.querySelector(".o_gantt_cell:has(.o_gantt_pill)").dataset.date, "2018-12-20 00:00:00");
        assert.strictEqual(getPillItemWidth(gantt.$('.o_gantt_pill_wrapper:eq(0)')), "calc(200% + 1px - 4px)");
    ...
    QUnit.test('date fields: drag a pill', async function (assert) {
        ...
        // move a pill by a cell to the left (-1 day)
        var cellWidth = gantt.$('.o_gantt_header_scale .o_gantt_header_cell:first')[0].getBoundingClientRect().width + 4;
        await testUtils.dom.dragAndDrop(
            gantt.$('.o_gantt_pill'),
            gantt.$('.o_gantt_pill'),
        ...
        assert.strictEqual(gantt.el.querySelector(".o_gantt_cell:has(.o_gantt_pill)").dataset.date, "2018-12-19 00:00:00");
        assert.strictEqual(getPillItemWidth(gantt.$('.o_gantt_pill_wrapper:eq(0)')), "calc(300% + 2px - 4px)");
    ...
    QUnit.test('date fields: popover', async function (assert) {
        ...
        assert.containsNone(gantt, 'div.popover', 'should not have a popover');
        gantt.$('.o_gantt_pill:contains("Task 1")').trigger('mouseenter');
        await testUtils.nextTick();
        assert.containsOnce($, 'div.popover', 'should have a popover');

        assert.strictEqual($('div.popover .flex-column span:nth-child(2)').text(), 	"20 Dec, 12:00 AM");
        assert.strictEqual($('div.popover .flex-column span:nth-child(3)').text(), "22 Dec, 12:00 AM");
    ...
    QUnit.test('date fields: dialog', async function (assert) {
        ...
        await testUtils.dom.triggerMouseEvent(gantt.$('.o_gantt_row_container .o_gantt_pill'), "click");
        await testUtils.nextTick();

        const $modal = $('.modal-lg');
        assert.strictEqual($modal.find('.modal-title').text(), "Open");
        assert.strictEqual($modal.length, 1, 'There should be one modal opened');
        assert.strictEqual($modal.find('[name=name] input').val(), 'Task 1');
        assert.strictEqual($modal.find('[name=start] input').val(), '12/20/2018');
        assert.strictEqual($modal.find('[name=stop] input').val(), '12/22/2018');
    ...
    QUnit.test("Only the task display name should be displayed if the task span more than two day even if the pill ends before 3am", async function (assert) {
        let initialDate = new Date(2024, 0, 1, 8, 0, 0);
        initialDate = new Date(initialDate.getTime() - initialDate.getTimezoneOffset() * 60 * 1000);
        const [task9_planned_dates, task10_planned_dates] = [
            {
                start: "2024-01-01 16:00:00",
                stop: "2024-01-02 01:00:00",
            },
        ...
        const pills = gantt.el.querySelectorAll(".o_gantt_pill_title");
        assert.strictEqual(pills[0].innerText, "4:00 PM - 1:00 AM (4h) - Task 9", "The date should be displayed since the pill is displayed in one day.");
        assert.strictEqual(pills[1].innerText, "Task 10", "The date should not be displayed since the pill is displayed in 2 days.")
        gantt.destroy();
    });
});
});
