...
var GanttRow = Widget.extend({
    ...
    _adaptPills: function () {
        ...
        const dateStartFieldIsDate = this.state.dateStartFieldType === "date";
        const dateStopFieldIsDate = this.state.dateStopFieldType === "date";
        this.pills.forEach(function (pill) {
            var pillStartDate = dateStartFieldIsDate ? pill[dateStartField].clone() : self._convertToUserTime(pill[dateStartField]);
            var pillStopDate = dateStopFieldIsDate ? pill[dateStopField].clone() : self._convertToUserTime(pill[dateStopField]);
            if (dateStopFieldIsDate) {
                pillStopDate.add({ day: 1 });
            }
    ...
    _generatePillLabels(scale) {
       // as localized yearless date formats do not exists yet in momentjs,
        ...
        this.pills.forEach((pill) => {
            ...
            const spanMoreThanOneDay = localStartDateTime.clone().startOf("day").diff(localEndDateTime.clone().startOf("day"), "days") < -1;

            const spanAccrossDays = spanMoreThanOneDay ||
                (localStartDateTime.clone().startOf("day").diff(localEndDateTime.clone().startOf("day"), "days") == -1 &&
