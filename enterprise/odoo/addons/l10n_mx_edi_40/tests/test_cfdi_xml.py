...
from odoo.addons.l10n_mx_edi.models.account_edi_format import CFDI_DATE_FORMAT
...
class TestEdiResults(TestMxEdiCommon):
        ...
        with freeze_time(self.frozen_today), \
            ...
            expected_etree = self.with_applied_xpath(
                ...
                '''
                    ...
                    <xpath expr="//Comprobante" position="attributes">
                      <attribute name="TipoCambio">2.000000</attribute>
                    </xpath>
                    <xpath expr="//Comprobante" position="attributes">
    ...
    @freeze_time('2017-01-02 6:59:00')
    def test_invoice_cfdi_timezone(self):
        with mute_logger('py.warnings'), \
            ...
            current_etree = self.get_xml_tree_from_string(generated_files[0])
            expected_etree = self.with_applied_xpath(
                self.get_xml_tree_from_string(self.expected_invoice_cfdi_values),
                '''
                    <xpath expr="//Comprobante" position="attributes">
                      <attribute name="Fecha">2017-01-01T23:59:00</attribute>
                    </xpath>
                '''
            )
            self.assertXmlTreeEqual(current_etree, expected_etree)
    ...
    def test_cfdi_date_with_timezone(self):
        def assert_cfdi_date(document, tz, expected_datetime=None):
            ...
            cfdi_node = etree.fromstring(document)
            cfdi_date_str = cfdi_node.get('Fecha')
            expected_datetime = (expected_datetime or self.frozen_today.astimezone(tz).replace(tzinfo=None).replace(microsecond=0)).strftime(CFDI_DATE_FORMAT)
            self.assertEqual(cfdi_date_str, expected_datetime)
        ...
        for address in addresses:
            ...
            with self.subTest(zip=address['zip']), \
                ...
                self._process_documents_web_services(self.invoice)
                invoice.l10n_mx_edi_cfdi_uuid = '123456786'
                generated_files = self._process_documents_web_services(invoice, {'cfdi_3_3'})
                assert_cfdi_date(generated_files[0], tz)
                ...
                self._process_documents_web_services(self.invoice)
                invoice.l10n_mx_edi_cfdi_uuid = '123456787'
                generated_files = self._process_documents_web_services(invoice, {'cfdi_3_3'})
                assert_cfdi_date(generated_files[0], tz, expected_datetime=date_in_the_past.replace(hour=23, minute=59, second=0))

                # Invoice in the present
                invoice = self._create_invoice()
                self._process_documents_web_services(invoice)
                invoice.l10n_mx_edi_cfdi_uuid = '123456788'
                generated_files = self._process_documents_web_services(invoice, {'cfdi_3_3'})
                assert_cfdi_date(generated_files[0], tz)
                ...
                invoice.l10n_mx_edi_cfdi_uuid = '123456788'
                generated_files = self._process_documents_web_services(invoice, {'cfdi_3_3'})
                assert_cfdi_date(generated_files[0], tz, expected_datetime=previous_send_time)
