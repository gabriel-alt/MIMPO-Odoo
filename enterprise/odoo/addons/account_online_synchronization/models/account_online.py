...
class AccountOnlineAccount(models.Model):
    ...
    journal_ids = fields.One2many('account.journal', 'account_online_account_id', string='Journal', domain="[('type', '=', 'bank'), ('company_id', '=', company_id)]")
            ...
            if online_account.journal_ids:
                if refresh:
                    ...
                    if status is not True:
                        # For ponto, if status is "updateCredentials", online account online_identifier is
                        # required to know which account needs credentials updated and to redirect the
                        # user to the ponto account reauth process.
                        include_param = {
                            'account_online_identifier': online_account.online_identifier
                        } if status == 'updateCredentials' else None
                        return self._open_iframe(status, include_param=include_param)
