...
class TestRealEstateBOEGeneration(TestEsAccountReportsCommon):
    ...
    def test_boe_mod_347(self):
        for _dummy in range(0, 2):
            invoice = self._create_invoice(move_type='out_invoice', partner_id=self.partner_agrolait.id, invoice_amount=50000, date_invoice=fields.Date.from_string('2020-12-15'))
