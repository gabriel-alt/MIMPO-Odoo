...
class AccountJournal(models.Model):
    ...
    def create_iso20022_credit_transfer(self, payments, batch_booking=False, sct_generic=False):
        ...
        if payments and pain_version == 'pain.001.001.09':
            if 'sepa_uetr' not in payments[0]:
                raise UserError(_("Please install SEPA pain.001.001.09 module to generate XML files in the new format."))
            if any(not payment['sepa_uetr'] for payment in payments):
    ...
    def _get_PmtTpInf(self, sct_generic=False, local_instrument=None):
        ...
        if self.env.context.get('sepa_payroll_sala'):
            ...
            InstrPrty = etree.Element("InstrPrty")
            InstrPrty.text = 'HIGH'
            PmtTpInf.insert(0, InstrPrty)
    ...
    def _get_RmtInf(self, payment, local_instrument=None):
        ...
        if local_instrument == 'CH01' or qr_iban:
            ...
            # Article 1411bis
            if self.company_id.account_fiscal_country_id.code == "BE" and self.env.context.get('sepa_payroll_sala'):
