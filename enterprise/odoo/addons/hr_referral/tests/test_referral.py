...
class TestHrReferral(TestHrReferralBase):
    ...
    def test_referral_no_hired_stage(self):
        ...
        self.assertEqual(self.job_dev.max_points, 76, "Max points for this job is 76 (points for the 3 'not hired stage' are ignored).")
        ...
        self.assertEqual(job_applicant.earned_points, 26, "He received points for stage 2 and stage 4 (in addition to stage 1 that he already received).")
        ...
        self.assertEqual(job_applicant.earned_points, 26, "Nothing change as it was only 'not hired stage'.")
    ...
    def test_referral_reset_applicant(self):
        ...
        self.assertEqual(applicant.referral_state, 'hired', 
                        "Applicant referral state should be 'hired'")
        
        # Reset the applicant
        applicant.reset_applicant()
        
        # Check if the referral state is reset to 'progress'
        self.assertEqual(applicant.referral_state, 'progress', 
                        "After reset_applicant, referral state should be 'progress'")
        
        # Test resetting from 'closed' state
        applicant.write({'referral_state': 'closed'})
        self.assertEqual(applicant.referral_state, 'closed', 
                        "Applicant referral state should be 'closed'")
    ...
    def test_referral_reset_applicant_batch(self):
        ...
        for applicant in applicants:
            self.assertEqual(applicant.referral_state, 'progress',
                            f"Applicant {applicant.name} should have referral_state 'progress' after reset")
