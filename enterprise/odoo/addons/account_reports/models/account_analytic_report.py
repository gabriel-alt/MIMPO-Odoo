...
class AccountReport(models.AbstractModel):
    ...
    def _prepare_lines_for_analytic_groupby(self):
        ...
        stored_move_line_fields = {(col[1], col[2]) for col in columns if col[0] == 'account_move_line'}
        stored_analytic_line_fields = {(col[1], col[2]) for col in columns if col[0] == 'account_analytic_line'}
        ...
        selected_fields = []
        for fname, data_type in stored_move_line_fields:
            if fname in changed_equivalence_dict:
                ...
                selected_fields.append(sql.SQL('to_jsonb(account_analytic_line.account_id) AS "account_move_line.analytic_distribution"'))
            else:
                selected_fields.append(sql.SQL('{table}."{fname}" AS "account_move_line.{fname}"').format(
                    table=sql.SQL("account_analytic_line") if (fname, data_type) in stored_analytic_line_fields else sql.SQL("account_move_line"),
                    fname=sql.SQL(fname),
                ))
        ...
        """).format(
            all_fields=sql.SQL(', ').join(sql.Identifier(fname) for fname, data_type in stored_move_line_fields),
