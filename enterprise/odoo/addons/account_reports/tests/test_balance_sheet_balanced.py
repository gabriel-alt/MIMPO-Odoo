...
REPORT_CONFIG = {
    <Balance Sheet report xmlid> : {
        ...
        'ignore': (optional) True if this report should not be tested (only for 16.0, will be removed in 17.0+)
    },
} '''
...
REPORT_CONFIG = {
    ...
    'l10n_dk_reports.account_balance_report_l10n_dk_balance': {
        'asset_line_ref': 'l10n_dk_reports.account_balance_report_l10n_dk_aktiver',
        'liability_line_ref': 'l10n_dk_reports.account_balance_report_l10n_dk_passiver',
    },
    ...
    'l10n_syscohada_reports.account_financial_report_syscohada_bilan': {
        ...
        'chart_template_refs': ['l10n_syscohada.syscohada_chart_template'],
    },
    'l10n_tr_reports.account_financial_report_l10n_tr_bilanco': {
        'asset_line_ref': 'l10n_tr_reports.account_financial_report_line_trbs_active',
        'liability_line_ref': 'l10n_tr_reports.account_financial_report_line_trbs_pasive',
        'ignore': True,
    },
    'l10n_tr_reports.account_financial_report_l10n_tr_ozet_bilanco': {
        'asset_line_ref': 'l10n_tr_reports.account_financial_report_line_tr_br_bs_active',
        'liability_line_ref': 'l10n_tr_reports.account_financial_report_line_tr_br_bs_pasive',
        'ignore': True,
    },
...
NON_TESTED_ACCOUNTS = {
    ...
    'l10n_syscohada.syscohada_chart_template': ['13'],
    'l10n_it.l10n_it_chart_template_generic': ['412', '413', '9102'],
    'l10n_pt.pt_chart_template': ['811'],
}
...
class TestBalanceSheetBalanced(TestAccountReportsCommon):
    ...
    def test_balance_sheet_balanced(self):
        ...
        installed_coas = self.env['account.chart.template'].search([])
        ...
        for coa in installed_coas:
            coa_ref = coa.get_external_id()[coa.id]
            with self.subTest(CoA=coa_ref):
                try:
                    ...
                    for report in available_reports:
                        ...
                        with self.subTest(Report=report_ref):
                            _logger.info('Testing report %s with CoA %s', report_ref, coa_ref)
    ...
    def _set_up_localization(self, coa):
        ...
        coa_setup_data = {}
        if coa in self.existing_companies.mapped('chart_template_id'):
            self.env.company = next(iter(self.existing_companies.filtered(lambda c: c.chart_template_id == coa)))
        ...
        generic_balance_sheet_options = self.env.ref('account_reports.balance_sheet').with_company(self.env.company)._get_options()
        available_report_ids = [variant['id'] for variant in generic_balance_sheet_options['available_variants']]
        ...
        report_ids_to_skip = [id_ for id_, report_ref in available_report_refs.items()
                              if coa_ref not in REPORT_CONFIG.get(report_ref, {}).get('chart_template_refs', [coa_ref])]
        ...
        for code in NON_TESTED_ACCOUNTS['all'] + NON_TESTED_ACCOUNTS.get(coa_ref, []):
    ...
    def _set_up_report(self, report):
        ...
        if report_config.get('ignore'):
            self.skipTest("Report marked as to ignore, skipping.")
