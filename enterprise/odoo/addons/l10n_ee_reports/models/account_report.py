...
class EstonianTaxReportCustomHandler(models.AbstractModel):
    ...
    def export_to_xml(self, options):
        ...
        for line_number in tax_line_numbers:
            tax_line = self.env.ref(tax_line_prefix + line_number, raise_if_not_found=False)
            if tax_line:
                tax_line_mapping[tax_line.id] = f'line_{line_number}'
    ...
    def _report_custom_engine_kmd_inf_common(self, options, current_groupby, next_groupby, kmd_inf_part):
        ...
        if kmd_inf_part == 'a':
            ...
            select_part_specific = """
                ...
                SUM(
                  CASE
                    -- Only base amounts reported on lines 1, 1_1, 2, 2_1, 2_2 of the tax report are reported here
                    WHEN account_account_tag.name->>'en_US' IN ('+1', '-1', '+1_1', '-1_1', '+2', '-2', '+2_1', '-2_1', '+2_2', '-2_2')
            ...
            # When we are in part A, we only want the invoices that have one or more lines reported on lines 1, 1¹, 2, 2¹, 2²
            # of the tax report
            groupby_clause += """
                HAVING SUM(
                         CASE
