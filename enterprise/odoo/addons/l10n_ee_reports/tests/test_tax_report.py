...
class EstonianTaxReportTest(AccountSalesReportCommon):
    ...
    def setUpClass(cls, chart_template_ref='l10n_ee.l10nee_chart_template'):
        ...
        cls.taxes = {
            ...
            'vat_in_22_s': cls.env['account.tax'].search([
                    ('name', '=', '22% S'),
                    ('type_tax_use', '=', 'purchase'),
                    ('company_id', '=', cls.company_data['company'].id)
                ], limit=1),
            ...
            'vat_in_13_s': cls.env['account.tax'].with_context(active_test=False).search([
                    ('name', '=', '13% S'),
                    ('type_tax_use', '=', 'purchase'),
                    ('company_id', '=', cls.company_data['company'].id)
                ], limit=1),
            ...
            'vat_out_13_s': cls.env['account.tax'].with_context(active_test=False).search([
                    ('name', '=', '13% S'),
                    ('type_tax_use', '=', 'sale'),
                    ('company_id', '=', cls.company_data['company'].id)
                ], limit=1),
    ...
    def test_generate_xml_purchase_13_tax(self):
        self.taxes['vat_in_22_g'].l10n_ee_kmd_inf_code = '11'
        moves = self.env['account.move'].create([
            {
                'move_type': 'in_invoice',
                'journal_id': self.company_data['default_journal_purchase'].id,
                ...
                'invoice_line_ids': [Command.create({
                    'name': 'PT1',
                    'price_unit': 500,
                    'tax_ids': self.taxes['vat_in_13_s'].ids,
                })],
            },
            {
                'move_type': 'in_invoice',
        ...
        moves = self.env['account.move'].create([
            {
                ...
                'invoice_line_ids': [Command.create({
                    'name': 'PT1',
                    'price_unit': 500,
                    'tax_ids': self.taxes['vat_out_13_s'].ids,
                })],
            },
            {
                'move_type': 'out_invoice',
