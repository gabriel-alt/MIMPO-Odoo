...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_get_common_cfdi_values(self, move):
        ...
        customer_rfc = customer._l10n_mx_edi_get_customer_rfc() if customer else False
    ...
    def _l10n_mx_edi_get_invoice_cfdi_values(self, invoice):
        ...
        :return:
        '''
        invoice_date = datetime.combine(invoice.invoice_date, time(hour=23, minute=59, second=00))

        # l10n_mx_edi_post_time is set when the invoice is sent to the PAC
        cfdi_date = min(invoice_date, invoice.l10n_mx_edi_post_time) if invoice.l10n_mx_edi_post_time else invoice_date
        ...
        cfdi_values = {
            ...
            **self._l10n_mx_edi_get_common_cfdi_values(invoice),
            ...
            'cfdi_date': cfdi_date.strftime(CFDI_DATE_FORMAT),
        ...
        elif invoice.amount_total:  # assumes that invoice.company_id.country_id.code == 'MX', as checked in '_is_required_for_invoice'
            cfdi_values['currency_conversion_rate'] = abs(invoice.amount_total_signed) / abs(invoice.amount_total)
        else:
            cfdi_values['currency_conversion_rate'] = self.env['res.currency']._get_conversion_rate(invoice.company_id.currency_id, invoice.currency_id, invoice.company_id, invoice.date)
    ...
    def _l10n_mx_edi_export_invoice_cfdi(self, invoice):
        ...
        * error:        An error if the cfdi was not successfuly generated.
        ...
        xsi_ns = cfdi_node.nsmap['xsi']
        schema_locations = cfdi_node.attrib[f"{{{xsi_ns}}}schemaLocation"].split()
        schema_parts = {ns: location for ns, location in zip(schema_locations[::2], schema_locations[1::2])}
        for ns in cfdi_node.nsmap:
            if ns != 'xsi' and not cfdi_node.xpath(f'//{ns}:*', namespaces=cfdi_node.nsmap):
                schema_parts.pop(cfdi_node.nsmap[ns])
        cfdi_node.attrib[f'{{{xsi_ns}}}schemaLocation'] = ' '.join(f"{ns} {location}" for ns, location in schema_parts.items())
        cfdi = etree.tostring(cfdi_node, pretty_print=True, xml_declaration=True, encoding='UTF-8')
    ...
    def _l10n_mx_edi_get_payment_cfdi_values(self, move):
        ...
        if currency == move.company_currency_id:
            ...
            payment_exchange_rate = float_round(
                ...
                precision_digits=6,
                rounding_method='UP',
            ) if total_amount_currency else 0.0
        ...
        cfdi_values['cfdi_payment_date'] = cfdi_payment_datetime.strftime(CFDI_DATE_FORMAT)
...
""" % {'boundary': boundary, 'cfdi_b64': cfdi_b64}
    ...
    def _l10n_mx_edi_get_sat_status(self, supplier_rfc, customer_rfc, total, uuid):
        ...
        status = response['Estado'] if hasattr(response, 'Estado') else ''
        return status
    ...
    def _get_move_applicability(self, move):
        ...
        amls = move.line_ids.filtered(lambda x: x.account_id.account_type == 'asset_receivable')
        if not amls or False in amls.mapped('reconciled'):
            return None

        reconciled_invoices = move._get_reconciled_invoices()
        if any(invoice.edi_state != 'sent' for invoice in reconciled_invoices):
            return None

        if (move.payment_id or move.statement_line_id).l10n_mx_edi_force_generate_cfdi \
            or 'PPD' in reconciled_invoices.mapped('l10n_mx_edi_payment_policy'):
            return {
    ...
    def _l10n_mx_edi_post_invoice(self, invoice):
        ...
        # == Set the posting time if not yet set == #
        if not invoice.l10n_mx_edi_post_time:
            invoice.l10n_mx_edi_post_time = fields.Datetime.to_string(invoice._l10n_mx_edi_get_current_mexico_time())
    ...
    def _l10n_mx_edi_post_payment(self, payment):
        ...
        # == Set the posting time if not yet set == #
        if not payment.l10n_mx_edi_post_time:
            payment.l10n_mx_edi_post_time = fields.Datetime.to_string(payment._l10n_mx_edi_get_current_mexico_time())
    ...
    def _l10n_mx_edi_cancel_payment(self, payment):
        ...
        res = getattr(self, '_l10n_mx_edi_%s_cancel' % pac_name)(payment.l10n_mx_edi_cfdi_uuid, payment.company_id,
                                                                 credentials, uuid_replace=uuid_replace)
        if res.get('errors'):
            if '205' in res['errors'][0]:
                res['errors'][1] += ", please try again in few minutes."
