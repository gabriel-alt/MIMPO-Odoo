...
class AccountMove(models.Model):
    ...
    def _l10n_mx_edi_get_current_mexico_time(self):
        ...
        if not cfdi_data:
            ...
            # For vendor bills, the CFDI XML must be posted in the chatter as an attachment.
                ...
                attachments = self.attachment_ids.filtered(
                    lambda x: x.mimetype == 'application/xml'
                    or x.mimetype == 'text/xml'
                    or x.name.lower().endswith('.xml')
                )
                if attachments:
    ...
    def _l10n_mx_edi_cron_update_sat_status(self):
        ...
        ]).sorted(key=lambda doc: doc.move_id.l10n_mx_edi_sat_status, reverse=True)
    ...
    def button_draft(self):
        ...
        for move in self:
            ...
            timezoned_today = move._l10n_mx_edi_get_current_mexico_time().date()
