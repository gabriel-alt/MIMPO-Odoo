...
import odoo.release
from odoo.tools import SQL, Identifier, float_is_zero
...
class SpanishMod349TaxReportCustomHandler(models.AbstractModel):
    ...
    def _custom_modelo349_common(self, options, current_groupby, key=None, move_type=None):
        ...
        query = SQL("""
            SELECT {select_from_groupby},
                ...
                SUM(currency_table.rate * account_move_line.balance * (CASE WHEN account_move_line__move_id.move_type IN ('out_refund', 'in_invoice') THEN -1 ELSE 1 END)) AS amount_total
            FROM {tables}
            ...
            account_move.id
        """).format(
            select_from_groupby=SQL("{group} AS grouping_key").format(group=Identifier('account_move_line', current_groupby)) if current_groupby else SQL('null'),
            tables=SQL(tables),
            where_clause=SQL(where_clause),
            group_by=SQL('account_move_line.{current_groupby},').format(current_groupby=SQL(current_groupby)) if current_groupby else SQL(''),
            ct_query=SQL(ct_query),
        )
        ...
        reversed_moves_dict = {move.id: move for move in self.env['account.move'].browse(reversed_entry_ids)}
    ...
    def export_boe(self, options):
        ...
        rslt += self._call_on_partner_sublines(options, 'l10n_es_reports.mod_349_consignment_sales_agreements_refunds', lambda report_data: self._write_type2_refund_records(options, report_data, current_company, 'R', 'l10n_es_reports.mod_349_consignment_sales_agreements'))
        rslt += self._call_on_partner_sublines(options, 'l10n_es_reports.mod_349_returns_goods_consignment_refunds', lambda report_data: self._write_type2_refund_records(options, report_data, current_company, 'D', 'l10n_es_reports.mod_349_returns_goods_consignment'))
        rslt += self._call_on_partner_sublines(options, 'l10n_es_reports.mod_349_substitutions_refunds', lambda report_data: self._write_type2_refund_records(options, report_data, current_company, 'C', 'l10n_es_reports.mod_349_substitutions'))
