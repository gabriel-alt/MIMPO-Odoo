...
QUnit.module(
    ...
    () => {
        ...
        QUnit.test("All accessible models can be selected in the relation filter", async function (assert) {
            ...
            assert.equal(model3.innerText, "Apple");
        ...
        QUnit.test("Choose any year in a year picker by clicking the picker", async function (assert) {
            ...
            assert.isVisible(target.querySelector('.datepicker-years'),
            "The picker should be displaying the years");

            const selectYearSpans = target.querySelectorAll("span[data-action='selectYear']");
        ...
        QUnit.test("Can add a record in a relational filter through the Search More button", async function (assert) {
            const spreadsheetData = {
                sheets: [{
                    cells: {
                        A1: { content: '=ODOO.FILTER.VALUE("customer")' },
                    }
                }],
                pivots: {
                    1: {
            ...
            const serverData = getBasicServerData();
            const maxId = Math.max(...serverData.models["product"].records.map((r) => r.id));
            for (let i = maxId + 1; i < maxId + 10; i++) { // force the apparition of "search More"
            ...
            const relation = serverData.models["product"].records[serverData.models["product"].records.length - 1];
            const relationName = relation.display_name;
            serverData.views["product,false,list"] = /* xml */ `
                <tree>
                    <field name="id"/>
            ...
            const { model } = await createSpreadsheet({
                ...
                mockRPC: async function (route, args) {
                    if (args.method === "name_search" && args.model === "product") {
                        return serverData.models["product"].records.map((r) => [r.id, r.display_name]);
                    }
                }
            });
            await openGlobalFilterSidePanel();
            ...
            await click(target, `td[data-tooltip="${relationName}"]`);

            assert.deepEqual(getCellValue(model, "A1"), relationName);
        });
    }
);
