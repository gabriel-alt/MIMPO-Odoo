...
class Project(models.Model):
    ...
    def _compute_assets_count(self):
        ...
        query_string, query_param = query.select(
            'jsonb_object_keys(account_asset.analytic_distribution) as account_id',
            'COUNT(DISTINCT(id)) as asset_count',
        )
        query_string = f'{query_string} GROUP BY jsonb_object_keys(account_asset.analytic_distribution)'
