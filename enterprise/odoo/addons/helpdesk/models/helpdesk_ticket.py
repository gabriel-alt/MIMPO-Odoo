...
class HelpdeskTicket(models.Model):
    ...
    def _sla_generate_status_values(self, slas, keep_reached=False):
        ...
        if keep_reached:
            exclude_slas_per_ticket = {
                value['ticket_id'][0]: value['sla_ids'] for value in self.env['helpdesk.sla.status']._read_group(
                    domain=[('reached_datetime', '!=', False), ('ticket_id', 'in', self.ids)],
                    fields=['sla_ids:array_agg(sla_id)'],
                    groupby=['ticket_id'],
                )
            }
        result = []
        for ticket in self:
            exclude_slas = self.env['helpdesk.sla'].browse(exclude_slas_per_ticket.get(ticket.id))
