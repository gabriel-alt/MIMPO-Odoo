...
class L10nNlTaxReportSBRWizard(models.TransientModel):
    ...
    def _get_sbr_identifier(self):
        return compact(self.env.company.vat)
    ...
    def send_xbrl(self):
        ...
        if not self.is_test:
            ...
            body = Markup("%s<br/>%s<br/>%s") % (
                _(
                    "The tax report from %(date_from)s to %(date_to)s was sent to Digipoort.",
                    date_from=format_date(self.env, self.date_from),
                    date_to=format_date(self.env, self.date_to),
                ),
                _("We will post its processing status in this chatter once received."),
                _("Discussion id: %(id)s", id=kenmerk),
            )
