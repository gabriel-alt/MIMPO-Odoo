...
const UserAgent = Class.extend(mixins.EventDispatcherMixin, ServicesMixin, {
    ...
    init(parent) {
        ...
        this._hasPendingReconnection = false;
        /**
    ...
    async _attemptReconnection(attemptCount = 0) {
        if (attemptCount > 5) {
            this.voip.triggerError(_t("The WebSocket connection was lost and couldn't be reestablished."));
            return;
        }
        if (this._hasPendingReconnection) {
            return;
        }
        this._hasPendingReconnection = true;
        try {
            await this.voip.userAgent.__sipJsUserAgent.reconnect();
            this.voip.userAgent.registerer.register();
            this._messaging.messagingBus.trigger("sip_error_resolved");
        } catch {
            setTimeout(
                () => this._attemptReconnection(attemptCount + 1),
    ...
    /**
     * Searches the database for a partner whose phone number matches the phone
     * number provided.
     *
     * @param {string} number The phone number.
     * @param {string|undefined} prefix A country code, e.g. 32 for a call from
     * Belgium or 1 for a call from the USA.
     * @returns {Object|null}
     */
    async _findCallerPartner(number, prefix) {
        const rpcPayload = {
            model: "res.partner",
            method: "search_read",
        ...
        const baseDomain = [
            "|",
            ["sanitized_phone", "=like", number],
            ["sanitized_mobile", "=like", number],
        ];
        let results = [];
        if (number.length < 6) {
            // really short number → probably an internal call, so only look for
            // partners with an associated user
            results = await this._rpc({
                ...rpcPayload,
                domain: [["user_ids", "!=", false], ...baseDomain],
            });
            if (results.length === 0) {
                // no match among internal users → retry on all partners
                results = await this._rpc({ ...rpcPayload, domain: baseDomain });
            }
        } else {
            const formattedNumber = this._formatPhoneNumber(number, prefix);
            const domain = formattedNumber !== number ? [
                "|", ...baseDomain, "|",
                ["sanitized_phone", "=like", formattedNumber],
                ['sanitized_mobile', "=like", formattedNumber],
            ] : baseDomain;
            results = await this._rpc({ ...rpcPayload, domain });
        }
        return results[0] || null;
    },
    /**
     * Tries to format the number to match the E.164 format, which is the format
     * used in the database (res.partner/sanitized_{mobile,phone}).
     * The E.164 format looks like this: +<country code><subscriber number>
     *
     * If it can't figure out the country code, it tries to replace it with the
     * PostgreSQL LIKE '%' wildcard instead.
     *
    ...
    _formatPhoneNumber(number, prefix) {
        ...
        // 00 is the international prefix recommended by the ITU-T.
        if (number.startsWith("00")) {
            return `+${number.substring(2, number.length)}`;
        }
        // USA: Calls between different area codes are usually prefixed
        // with 1. Conveniently, the country code for the USA also
        // happens to be 1, so we just need to add the + symbol to
        // format it like an international call.
        if (number.startsWith("1")) {
            return `+${number}`;
        }
        if (prefix) {
            return `+${prefix}${number.substring(1, number.length)}`;
        }
        // 0 is the ITU-T recommended national prefix. This means that a number
        // starting with 0 will identify a national call in most countries.
    ...
    _onDisconnect(error) {
        ...
        console.error(error);
        this.voip.triggerError(_t("The websocket connection to the server has been lost. Attempting to reestablish the connection…"));
    ...
    async _onInvite(inviteSession) {
        ...
        const callerData = await this._findCallerPartner(number, inviteSession.remoteIdentity.uri.type);
        if (callerData) {
            name = callerData.display_name;
            incomingCallParams.partnerId = callerData.id;
        }
        let content;
        if (name) {
