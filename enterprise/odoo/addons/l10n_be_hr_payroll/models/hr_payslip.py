...
class Payslip(models.Model):
    ...
    def _get_base_local_dict(self):
        ...
        res.update({
            ...
            'compute_termination_fees_withholding_taxes': compute_termination_fees_withholding_taxes,
...
def compute_employment_bonus_employees(payslip, categories, worked_days, inputs):
    ...
    if payslip.env.context.get('salary_simulation'):
        ...
        worked_days = payslip.dict.worked_days_line_ids.filtered(lambda wd: wd.code not in ['LEAVE300', 'LEAVE301', 'MEDIC01'])
...
def compute_double_holiday_withholding_taxes(payslip, categories, worked_days, inputs):
    ...
    return _compute_withholding_taxes_after_child_allowances(payslip, rates, gross)
...
def compute_termination_fees_withholding_taxes(payslip, categories, worked_days, inputs):
    # See: https://www.securex.eu/lex-go.nsf/vwReferencesByCategory_fr/52DA120D5DCDAE78C12584E000721081?OpenDocument
    if payslip.date_from.year >= 2024:

        rates = payslip.rule_parameter('termination_fees_pp_rates')
        gross = categories.GROSS

        return _compute_withholding_taxes_after_child_allowances(payslip, rates, gross, apply_reduction=False)

    return compute_withholding_taxes(payslip, categories, worked_days, inputs)


def _compute_withholding_taxes_after_child_allowances(payslip, rates, gross, apply_reduction=True):
...
def compute_representation_fees(payslip, categories, worked_days, inputs):
    ...
    if categories.BASIC and (
        not all(day.work_entry_type_id.is_leave for day in worked_days.dict.values())
        or payslip.env.context.get('salary_simulation')
    ):
        contract = payslip.contract_id
...
def _compute_holiday_pay_recovery(payslip, categories, worked_days, inputs, recovery_type):
    """
    ...
    all_payslips_during_civil_year = payslip.dict.env['hr.payslip'].search([
            ('employee_id', '=', employee.id),
            ('date_from', '>=', date(payslip.dict.date_from.year, 1, 1)),
            ('date_to', '<=', date(payslip.dict.date_from.year, 12, 31)),
            ('state', 'in', ['done', 'paid']),
        ])
    ...
    remaining_amount = max_amount_to_recover - employee['l10n_be_holiday_pay_recovered_' + recovery_type]
...
def compute_holiday_pay_recovery_n(payslip, categories, worked_days, inputs):
    return _compute_holiday_pay_recovery(payslip, categories, worked_days, inputs, 'n')

def compute_holiday_pay_recovery_n1(payslip, categories, worked_days, inputs):
    return _compute_holiday_pay_recovery(payslip, categories, worked_days, inputs, 'n1')
