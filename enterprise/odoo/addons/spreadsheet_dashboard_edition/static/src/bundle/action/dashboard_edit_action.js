...
class DashboardEditAction extends AbstractSpreadsheetAction {
    setup() {
        super.setup();
        this.notificationMessage = _t("New dashboard created");
        /** @type {State} */
    ...
    async _onMakeCopy({ data, display_thumbnail }) {
        const defaultValues = {
            raw: JSON.stringify(data),
            spreadsheet_snapshot: false,
            display_thumbnail,
        };
        const copyId = await this.orm.call("spreadsheet.dashboard", "copy", [this.resId], {
            default: defaultValues,
        });
        this._openSpreadsheet(copyId);
    }
    ...
    async _onSpreadsheetSaved({ display_thumbnail }) {
        await this.orm.write("spreadsheet.dashboard", [this.resId], { display_thumbnail });
    }
}
