...
class AccountMove(models.Model):
    ...
    @api.depends('partner_expense_line_ids', 'partner_expense_line_ids.total_expense')
    def _compute_expense_amount(self):
        """ Compute Expense Amount """

        for move in self:
            total_expense = 0.0
            for line in move.partner_expense_line_ids.filtered(lambda r: not r.to_agency):
                total_expense += line.subtotal_expense
            move.update({'total_expense': total_expense})

    @api.depends('sir_id', 'date', 'total_expense', 'amount_total', 'state')
    ...
    def generate_cga_expense_entries(self):
        ...
        for line in self.invoice_line_ids.filtered(
                ...
                and float_compare(
                    abs(r.balance), abs(r.origin_expense_line_id.subtotal_expense),
                    precision_rounding=self.currency_id.rounding) != 0):
            diff += abs(line.origin_expense_line_id.subtotal_expense) - abs(line.balance)
...
class AccountMoveLine(models.Model):
    ...
    total_expense = fields.Monetary("Total Expense", compute="_compute_expense_amount",
                                    currency_field="company_currency_id", store=True)
    subtotal_expense = fields.Monetary("Subtotal Expense", compute="_compute_expense_amount",
                                    currency_field="company_currency_id", store=True)
