...
class ResPartner(models.Model):
    ...
    def write(self, vals):
        ...
        if ctx.get('res_partner_search_mode', False) and ctx.get('res_partner_search_mode', False) == 'customer' and not ctx.get('from_create', False):
            ...
            reco_vals = {
                'ClvCliente': self.id,
                'RFC': self.vat,
                'CURP': curp_representante_legal if (self.type_company == 'moral' and self.company_type == 'company') else self.l10n_mx_edi_curp,
    ...
    def unlink(self):
        ...
        for rec in self:
            ...
            # response = requests.delete(url+'Delete?Clave='+rec_id, headers=headers, data=body)
