...
class CveTransporte(models.Model):
    ...
    total_taxes = fields.Float(
        string="Total de impuestos pagados en efectivo del pedimento"
    )
    ...
    amount_insurance = fields.Float(
        string="Importe de incrementables del pedimento (seguros)"
    )
    amount_expense = fields.Float(
        string="Importe de incrementables del pedimento (Gastos)"
    )
    amount_ded_other = fields.Float(
        string="Importe de otros incrementables del pedimento"
    )
    ...
    pece_account = fields.Boolean(string="Pagado con cuenta PECE")
    pedimento_date = fields.Char(string="Fecha en que se pago el pedimento")
    custom_house = fields.Char(string="Clave de aduana")
    ...
    def _name_search(
        ...
        if name:
            ...
            try:
                ref_number = int(name)
                domain = ["|", ("ref_id", "=", ref_number), ("number", operator, name)]
            except:
    ...
    def create_analytic(self):
        ...
        for record in self:
            ...
            _logger.warning(f"Vals to create Analytic account: {vals}")

            analytic = self.env["account.analytic.account"].create(vals)
            if not record.partner_id and vals.get("partner_id") != 0:
                record.write(
                    {
            ...
            elif not record.partner_id and vals.get("partner_id") == 0:
                partner_id = self.env["res.partner"].search(
                    [("partner_ref", "=", vals.get("customer"))], limit=1
                )
                record.write(
                    {"partner_id": partner_id.id, "account_analytic_id": analytic.id}
                )
            else:
    ...
    def default_analytic_plan_validation(self):
        ...
        if not company.sir_analytic_plan_id:
            raise ValidationError(
                _("Please Define Default Analytic Plan for SIR Reference Creation!")
            )
    ...
    def create(self, vals_list):
        ...
        sir_id = self.env["sir.ref"].search([("number", "=", sir_number)], limit=1)
        if sir_id:
    ...
    def action_view_move(self):
        ...
        result["context"] = {
            "default_sir_id": self.id,
            "search_default_sir_id": self.id,
        }
