...
def _prepare_edi_vals_to_export(self):
    ...
    for index, line in enumerate(
        self.invoice_line_ids.filtered(
            lambda line: line.display_type == 'product'), start=1):
...
class AccountMove(models.Model):
    ...
    total_expense = fields.Monetary(
        "Total Expense (Company Currency)", compute="_compute_expense_amount", store=True)
    ...
    trusted_fund_statement_line_ids = fields.Many2many(
        "account.bank.statement.line", "account_move_trusted_fund_statement_line_rel",
        "move_id", "statement_line_id", compute="_compute_trusted_fund_balance", store=True)
    invoice_expense_ids = fields.Many2many(
        'account.move', 'invoice_expense_account_move_rel',
        'bill_id', 'invoice_expense_id', compute="_compute_invoice_expense_ids")
    ...
    l10n_mx_edi_payment_policy_draft = fields.Selection(string='Payment Policy Draft',
        selection=[('PPD', 'PPD'), ('PUE', 'PUE')], readonly=True)

    @api.depends('partner_id', 'move_type', 'invoice_date_due', 'invoice_date',
                 'invoice_payment_term_id', 'invoice_payment_term_id.line_ids')
    def _compute_l10n_mx_edi_payment_policy(self):
        ...
        for move in non_draft:
            if move.is_invoice(include_receipts=True) and \
                move.partner_id and move.partner_id.l10n_mx_edi_payment_policy:
                move.l10n_mx_edi_payment_policy = move.partner_id.l10n_mx_edi_payment_policy
    ...
    def _compute_invoice_expense_ids(self):
        for move in self:
            ...
            move.update({
                'invoice_expense_ids': [
                    (6, 0, move_ids and move_ids.ids or [])]})

    @api.depends('partner_expense_line_ids',
    ...
    def _compute_expense_amount(self):
        ...
        for move in self:
            ...
            move.update({'total_expense': total_expense,
    ...
    def _compute_trusted_fund_balance(self):
        ...
        for move in self:
            ...
            move.update({'remaining_trusted_fund_balance': move.currency_id.round(
    ...
    def generate_cga_expense_entries(self):
        ...
        type_sign = 1 if float_compare(
            fund_balance, 0.0,
            precision_rounding=self.currency_id.rounding) <= 0 else -1
        ...
        for line in self.partner_expense_line_ids:
            ...
            taxes = line.product_id.with_company(self.company_id).taxes_id.ids \
                if line.product_id and line.move_id.to_agency else []
            ...
            if line.move_id.to_agency and line.product_id:
                ...
                account = accounts['income'].id if accounts.get('income') else line.account_id
            ...
            if line.currency_id != self.currency_id:
                ...
                price = line.currency_id.with_context(
                    from_bill_line=line, to_expense_move=self)._convert(
            ...
            for vl in vals_list:
                ...
                if account != vl.get('account_id') or \
                    line_name != vl.get('name') or \
                    line.quantity != vl.get('quantity') or \
                    line.discount != vl.get('discount'):
                    continue
        ...
        ln_vals = []
        for line in self.invoice_line_ids.filtered(lambda r: r.cga_line):
            if line.origin_expense_line_id \
                    and not line.origin_expense_line_id.move_id.to_agency \
                    and line.origin_expense_line_id.account_id != line.account_id:
                ln_vals.append((1, line.id,
                                {'account_id': line.origin_expense_line_id.account_id.id}))
            elif line.trust_fund_account_id and line.trust_fund_account_id != line.account_id:
                ln_vals.append((1, line.id, {'account_id': line.trust_fund_account_id.id}))

        # Currency rounding correction
        diff = 0.0
        for line in self.invoice_line_ids.filtered(
                lambda r: r.origin_expense_line_id and float_compare(
                    r.origin_expense_line_id.currency_rate, r.origin_expense_line_id.exchange_rate,
                    precision_rounding=self.currency_id.rounding) != 0 \
                and float_compare(
                    abs(r.balance), abs(r.origin_expense_line_id.balance),
                    precision_rounding=self.currency_id.rounding) != 0):
        ...
        if not float_is_zero(diff, precision_rounding=self.currency_id.rounding):
            ...
            vals = {'name': _('Currency exchange rate difference'),
                    'analytic_distribution': {str(self.sir_id.account_analytic_id.id):100},
                    'quantity': 1.0,
    ...
    def action_link_expense(self):
        ...
        if not self.sir_id:
            return
        domain = [('sir_id', '=', self.sir_id.id), ('expense_move_id', '=', False),
                  ('move_id.state', '=', 'posted'), ('move_type', '=', 'in_invoice'),
                  ('product_id.concepto_corresponsal', '=', False),
                  ('display_type', '=', 'product')]
        bill_lines = self.env['account.move.line'].search(domain)
        if not bill_lines:
            msg = _('There are no published documents for reference SIR %s' \
                ' or were already used in a previous invoice',self.sir_id.number)
    ...
    def _prepare_invoice_aggregated_taxes(self, filter_invl_to_apply=None,
        ...
        base_lines = [
            ...
            for x in self.line_ids.filtered(
                lambda x: (not x.cga_line or (x.cga_line and x.product_id)) and x.display_type == 'product' \
        ...
        for base_line in base_lines:
            to_update_vals, tax_values_list = self.env[
                'account.tax']._compute_taxes_for_single_line(base_line)
...
class AccountMoveLine(models.Model):
    ...
    total_expense = fields.Monetary(
        "Total Expense", compute="_compute_expense_amount",
        currency_field="company_currency_id", store=True)
    subtotal_expense = fields.Monetary(
        "Subtotal Expense (company currency)", compute="_compute_expense_amount",
        currency_field="company_currency_id", store=True)
    expense_move_currency_id = fields.Many2one(
        'res.currency', string='Expense Move Currency',
        compute='_compute_expense_move_currency_id', readonly=True,
        store=True, precompute=True,
    )
    total_expense_currency = fields.Monetary(
        "Total Expense Currency", compute="_compute_expense_price_move_currency_id",
        currency_field="expense_move_currency_id", store=True)
    subtotal_expense_currency = fields.Monetary(
        "Subtotal Expense", compute="_compute_expense_price_move_currency_id",
        currency_field="expense_move_currency_id", store=True)
    price_unit_expense_currency = fields.Float(
        string='Expense Unit Price',
        compute="_compute_expense_price_move_currency_id", store=True,
    ...
    origin_expense_line_ids = fields.Many2many(
        'account.move.line', 'origin_expense_account_move_line_rel',
        'inv_line_id', 'bill_line_id', copy=False, readonly=True)
    invoice_expense_line_ids = fields.Many2many(
        'account.move.line', 'origin_expense_account_move_line_rel',
        'bill_line_id', 'inv_line_id', copy=False, readonly=True)
    cga_line = fields.Boolean(help="Line Generated from CGA Operation", copy=False, readonly=True)
    to_agency = fields.Boolean(help="To Agency from CGA Operation", copy=False, readonly=True)
    trust_fund_account_id = fields.Many2one('account.account')
    expense_price_unit = fields.Float(
        string='Expense Unit Price (Bills)',
        compute="_compute_expense_price_unit", store=True,
        readonly=False, precompute=True, digits='Product Price',)
    expense_description = fields.Char(
        string='Expense Description', compute='_compute_expense_description',
        store=True, readonly=False, precompute=True)
    expense_bill_currency_id = fields.Many2one(
        'res.currency', string='Expense Bill Currency',
    ...
    def _compute_expense_bill_currency_id(self):
        for line in self:
            ...
            if line.origin_expense_line_ids and \
                line.origin_expense_line_ids[0].currency_id != line.currency_id:
                cur_id = line.origin_expense_line_ids[0].currency_id
            line.update({'expense_bill_currency_id': cur_id.id})

    @api.depends('origin_expense_line_ids', 'expense_bill_currency_id',
                 'display_type', 'amount_currency')
    ...
    def _compute_expense_amount(self):
        ...
        for line in self:
            ...
            if line.expense_move_id and line.currency_id != line.expense_move_id.currency_id:
            ...
            line.update({'total_expense': total_expense,
    ...
    def _compute_expense_price_unit(self):
        ...
        for line in self:
            ...
            if line.expense_move_id and \
                line.currency_id != line.expense_move_id.currency_id:
    ...
    def _compute_account_id(self):
        ...
        product_lines = self.filtered(
            ...
            not l.move_id.to_agency and l.move_id.sir_id and \
        ...
        for line in product_lines:
            ...
            if customer_id:
                customer_id = customer_id.with_company(
        ...
        cga_lines = self.filtered(
            lambda l: l.display_type == 'product' and \
            l.move_id.sir_id and l.cga_line and \
            l.move_id.move_type in ['out_invoice', 'out_refund'])
        for cl in cga_lines:
            if not cl.origin_expense_line_ids and not cl.trust_fund_account_id:
                continue
            if cl.origin_expense_line_id and \
                not cl.origin_expense_line_id.move_id.to_agency and \
                cl.origin_expense_line_id.account_id != cl.account_id:
                cl.account_id = cl.origin_expense_line_id.account_id
            elif cl.origin_expense_line_ids and \
                not cl.origin_expense_line_ids[0].move_id.to_agency and \
                cl.origin_expense_line_ids[0].account_id != cl.account_id:
