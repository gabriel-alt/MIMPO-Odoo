...
class AccountMove(models.Model):
    ...
    down_payment_invoice_id = fields.Many2one(
        ...
        domain="[('state', '=', 'posted'), ('related_dp_invoice_ids', '=', False), ('downpayment_invoice', '=', True), ('partner_id', 'child_of', commercial_partner_id), ('company_id', '=', company_id), ('sir_id', '=?', sir_id)]")
    ...
    def _onchange_sir(self):
        ...
        for move in self:
            if move.sir_id.move_ids and move.move_type in [
...
class AccountEdiDocument(models.Model):
    ...
    def _process_documents_web_services(self, job_count=None, with_commit=True):
        res = super(AccountEdiDocument, self)._process_documents_web_services(
            job_count=job_count, with_commit=with_commit)
        for doc in self:
            if doc.move_id.l10n_mx_edi_cfdi_uuid and \
                doc.move_id.down_payment_invoice_id and \
                doc.move_id.down_payment_invoice_id.reversal_dp_inv_ids:
                new_cfdi = '07|' + doc.move_id.l10n_mx_edi_cfdi_uuid
                for related_dp_inv in doc.move_id.down_payment_invoice_id.reversal_dp_inv_ids:
                    related_dp_inv.write({
                        'l10n_mx_edi_origin': new_cfdi})
        return res