...
class AccountMove(models.Model):
    ...
    def _filter_move_sir_reference(self, state):
        filter_move = self
        if state == 'close':
            filter_move = self.filtered(
                lambda s: 'CGA' not in s.journal_id.code or not s.downpayment_invoice)
        return filter_move
    ...
    def update_sir_reference_state(self):
        ...
        filter_move = self._filter_move_sir_reference(state)
        if filter_move:
    ...
    def _post(self, soft=True):
        ...
        self.filtered(lambda r: r.move_type in ['out_invoice', 'out_refund'] \
