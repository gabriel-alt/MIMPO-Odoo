...
class AccountMove(models.Model):
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            ...
            #(2024/20/12) --> rate_ids may be False, it's not required for all companies.
            if rate_ids:
    ...
    def rate_compute(self):
        ...
        if inv_id and self._context.get('active_model') == 'acount.move':
        ...
        if product_id_1:
            ...
            if self.currency_id != currency_id_1:
                currency_amount = currency_id_1._convert(
        ...
        if product_id_2:
            ...
            if self.currency_id != currency_id_2:
                currency_amount = currency_id_2._convert(
                    from_amount=currency_amount,
                    to_currency=self.currency_id,
                    company=self.company_id,
                    date=self.invoice_date or self.date \
        ...
        if product_id_3:
            for product in product_id_3:
                ...
                if self.currency_id != product.currency_id:
                    currency_amount = product.currency_id._convert(
                        from_amount=currency_amount,
                        to_currency=self.currency_id,
                        company=self.company_id,
                        date=self.invoice_date or self.date \
