...
class SaleOrder(models.Model):
    ...
    sale_manual_currency_rate_active = fields.Boolean('Aplicar tasa manual')
    sale_manual_currency_rate = fields.Float('Tipo de cambio', digits=0)
    inverse_sale_manual_currency_rate = fields.Float(
        'Tipo de cambio', digits=0,
        compute="_compute_inverse_sale_manual_currency_rate",
        inverse="_inverse_inverse_sale_manual_currency_rate",)

    @api.depends('sale_manual_currency_rate')
    def _compute_inverse_sale_manual_currency_rate(self):
        for record in self:
            if not record.sale_manual_currency_rate:
                record.sale_manual_currency_rate = 1.0
            inverse_rate = 1.0 / record.sale_manual_currency_rate
            if tools.float_compare(
                record.inverse_sale_manual_currency_rate, inverse_rate,
                precision_digits=6):
                record.inverse_sale_manual_currency_rate = inverse_rate

    @api.onchange('inverse_sale_manual_currency_rate')
    def _inverse_inverse_sale_manual_currency_rate(self):
        for record in self:
            if not record.inverse_sale_manual_currency_rate:
                record.inverse_sale_manual_currency_rate = 1.0
            inverse_rate = 1.0 / record.inverse_sale_manual_currency_rate
            if tools.float_compare(
                record.sale_manual_currency_rate, inverse_rate,
                precision_digits=6):
                record.sale_manual_currency_rate = inverse_rate
